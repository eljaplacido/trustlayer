...
func TestDisclosureShownRoundTrips(t *testing.T) {
	ev := NewEvent("discloser", "s1", EventDisclosureShown,
		WithPayload(map[string]any{"disclosure_type": "ai_generated", "user_notice": "This response was generated by AI"}),
		WithCynefin(CynefinComplex),
	)
	body, err := json.Marshal(ev)
	if err != nil {
		t.Fatalf("marshal: %v", err)
	...
	if back.EventType != EventDisclosureShown {
		t.Errorf("event_type = %q, want DISCLOSURE_SHOWN", back.EventType)
	}
	if back.AgentID != "discloser" || back.SessionID != "s1" {
		t.Errorf("identity lost: %+v", back)
	}
	if tn, _ := back.Payload["disclosure_type"].(string); tn != "ai_generated" {
		t.Errorf("payload.disclosure_type = %v, want ai_generated", tn)
	}
}

func TestContentMarkedRoundTrips(t *testing.T) {
	ev := NewEvent("marketer", "s2", EventContentMarked,
		WithPayload(map[string]any{"marking_type": "watermark", "confidence": 0.97}),
		WithCynefin(CynefinComplex),
	)
	body, err := json.Marshal(ev)
	if err != nil {
	...
	if back.EventType != EventContentMarked {
		t.Errorf("event_type = %q, want CONTENT_MARKED", back.EventType)
	}
	if back.AgentID != "marketer" || back.SessionID != "s2" {
		t.Errorf("identity lost: %+v", back)
	}
	if mt, _ := back.Payload["marking_type"].(string); mt != "watermark" {
		t.Errorf("payload.marking_type = %v, want watermark", mt)
	}
}
